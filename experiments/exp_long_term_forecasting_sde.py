    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, batch in enumerate(train_pbar):
                ...
                rec_weight = float(getattr(self.args, 'sde_rec_weight', 0.25))
                spectral_weight = float(
                    getattr(self.args, 'sde_spectral_weight', 0.75)
                )
                loss = rec_weight * criterion(outputs, batch_y)
                if spectral_weight > 0:
                    spectral_error = (
                        torch.fft.fft(outputs, dim=1)
                        - torch.fft.fft(batch_y, dim=1)
                    )
                    loss = loss + spectral_weight * spectral_error.abs().mean()

                # ---- Mixed loss: MAE + DiffLoss ----
                mae_weight = float(getattr(self.args, 'mae_weight', 0.0))
                diff_loss_weight = float(getattr(self.args, 'diff_loss_weight', 0.0))
                if mae_weight > 0:
                    loss = loss + mae_weight * F.l1_loss(outputs, batch_y)
                if diff_loss_weight > 0:
                    pred_diff = outputs[:, 1:, :] - outputs[:, :-1, :]
                    true_diff = batch_y[:, 1:, :] - batch_y[:, :-1, :]
                    loss = loss + diff_loss_weight * criterion(pred_diff, true_diff)
